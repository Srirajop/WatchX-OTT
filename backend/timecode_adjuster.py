...
def parse_offset_input(value: str) -> float | None:
    ...
    # Try plain float — strip a trailing unit suffix like 's' or 'sec' first
    # (docstring examples like "+1.5s" should work, not just "+1.5")
    numeric_part = re.sub(r'(s|sec|secs|seconds)\s*$', '', raw, flags=re.IGNORECASE).strip()
    try:
        return sign * float(numeric_part)
    except ValueError:
        pass
